...
impl OpenFangKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        let driver_config = DriverConfig {
            ...
            base_url: config.provider_urls
                .get(&config.default_model.provider)
                .cloned()
                .or_else(|| config.default_model.base_url.clone()),
        };
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        let primary = if agent_provider == default_provider && !has_custom_key && !has_custom_url {
            ...
            let base_url = if has_custom_url {
                ...
                self.config.provider_urls.get(agent_provider).cloned()
                    .or_else(|| self.config.default_model.base_url.clone())
            } else {
                // Let create_driver() use the target provider's default base URL unless overridden
                self.config.provider_urls.get(agent_provider).cloned()
            };
